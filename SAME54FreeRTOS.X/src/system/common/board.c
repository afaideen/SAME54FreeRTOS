...
//}
bool board_sw_pressed_rtos(sw_t *sw)
{
#ifdef USE_FREERTOS
    
    
    static sw_state_t sw0 = { .st = SW_ST_IDLE, .t0 = 0 };

    bool raw;

    switch (sw->id) {
        case SW0:
            raw = SW0_Pressed();      /* true when pressed */
            break;
        default:
            return false;
    }
    ...
    TickType_t db  = pdMS_TO_TICKS(sw->t_debounce);

    switch (sw0.st)
    {
        case SW_ST_IDLE:
            if (raw) {
                sw0.st = SW_ST_DEBOUNCE_PRESS;
                sw0.t0 = now;
            }
            break;

        case SW_ST_DEBOUNCE_PRESS:
            if (!raw) {
                /* bounced back -> cancel */
                sw0.st = SW_ST_IDLE;
            } else if ((now - sw0.t0) >= db) {
                /* stable press qualified -> fire one-shot event */
                sw0.st = SW_ST_WAIT_RELEASE;
                sw->cnt++;

                UART2_DMA_Log("SW0 PRESSED (>%lu ms)\r\n", (unsigned long)sw->t_debounce);
                return true;
            }
            break;

        case SW_ST_WAIT_RELEASE:
            if (!raw) {
                sw0.st = SW_ST_DEBOUNCE_RELEASE;
                sw0.t0 = now;
            }
            break;

        case SW_ST_DEBOUNCE_RELEASE:
            if (raw) {
                /* bounced -> still pressed */
                sw0.st = SW_ST_WAIT_RELEASE;
            } else if ((now - sw0.t0) >= db) {
                /* stable release -> re-arm */
                sw0.st = SW_ST_IDLE;
            }
            break;
