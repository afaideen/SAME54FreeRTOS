 ...
 * ============================================================================ */
static StaticSemaphore_t s_uart2_dma_log_mtx_buf;
static SemaphoreHandle_t s_uart2_dma_log_mtx = NULL;
...
//    s_uart2_log_prev_cyc  = millis();
    ...
    s_uart2_dma_log_mtx = xSemaphoreCreateMutexStatic(&s_uart2_dma_log_mtx_buf);
}
...
{
    /* If someone ever calls this from an ISR, just drop (mutex is not ISR-safe). */
    if (__get_IPSR() != 0U) {
        dma_log_dropped++;
        return false;
    }

    /* Serialize task callers (non-blocking) */
    if (s_uart2_dma_log_mtx != NULL)
    {
        if (xSemaphoreTake(s_uart2_dma_log_mtx, 0) != pdTRUE)
        {
            dma_log_dropped++;
            return false;
        }
    ...
    if (s_uart2_dma_log_mtx != NULL)
    {
        (void)xSemaphoreGive(s_uart2_dma_log_mtx);
    }
